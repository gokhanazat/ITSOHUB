...
actual object PdfGenerator {
    actual fun generateShowroomCatalog(products: List<ShowroomProduct>, company: B2BCompany?): ByteArray {
        ...
        coverCanvas.drawText("ÜRÜN KATALOĞU", 50f, 150f, titlePaint)
        
        bodyPaint.color = Color.WHITE
        bodyPaint.textSize = 18f
        coverCanvas.drawText(company?.name ?: "ITSO GLOBAL Platform", 50f, 200f, bodyPaint)
        ...
        var coverY = 400f
        company?.let {
            coverCanvas.drawText("ŞİRKET BİLGİLERİ", 50f, coverY, bodyBoldPaint)
            coverY += 30f
            ...
            coverY += 20f
            coverCanvas.drawText("Ülke: ${it.country}", 50f, coverY, bodyPaint)
        }
        ...
        products.forEachIndexed { index, product ->
            ...
            // Description
            canvas.drawText("Açıklama:", 40f, currentY, bodyBoldPaint)
            currentY += 25f
            ...
            canvas.drawText("Bu katalog ITSO GLOBAL Platform üzerinden oluşturulmuştur.", 40f, 825f, footerPaint)
        ...
        if (products.isEmpty()) {
            val page = document.startPage(pageInfo)
            val canvas = page.canvas
            canvas.drawText("Seçili ürün bulunamadı.", 40f, 100f, bodyPaint)
            document.finishPage(page)
        }
